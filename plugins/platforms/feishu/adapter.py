...
class FeishuAdapter(BasePlatformAdapter):
    ...
    async def _send_raw_message(
        ...
        *,
        ...
        # For topic/thread messages that fell back from reply→create, reply to
        # the thread root message so it lands in the correct topic.  The
        # thread_id (om_xxx) IS a valid message_id — it's the topic's root
        # message — so we can reply to it directly with reply_in_thread=True.
